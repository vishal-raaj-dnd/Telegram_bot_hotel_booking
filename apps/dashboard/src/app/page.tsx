...
import { 
  ...
  Clock, XCircle, HelpCircle, Eye, EyeOff, Check, CreditCard, ShieldAlert
} from 'lucide-react';
...
export default function Home() {
  ...
  // App state
  const [activeTab, setActiveTab] = useState<'dashboard' | 'rooms' | 'bookings' | 'settings' | 'billing'>('dashboard');
  ...
  const getRemainingTrialDays = () => {
    if (!hotelConfig?.createdAt) return 0;
    const createdAt = new Date(hotelConfig.createdAt);
  ...
  const handleSubscribe = async (plan: 'starter' | 'pro' | 'enterprise') => {
    ...
    try {
      const res = await ApiService.createCheckoutSession(token, plan);
      ...
      setBillingError(err.message || 'Failed to start Stripe subscription flow.');
    } finally {
  ...
  const handleManageBilling = async () => {
    ...
    setBillingError(null);
    try {
      const res = await ApiService.createPortalSession(token);
      if (res.success && res.data?.url) {
        window.location.href = res.data.url;
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#09090b] text-[#f4f4f5]">
      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto space-y-6">
        ...
        {hotelConfig?.subscriptionStatus === 'trialing' && (
          <div className="relative bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 border border-indigo-500/20 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 overflow-hidden">
            <div className="absolute top-0 right-0 w-[150px] h-[150px] bg-indigo-500/5 rounded-full blur-[40px] -mr-10 -mt-10" />
            <div className="flex items-center gap-3 relative z-10">
              <div className="w-10 h-10 rounded-lg bg-indigo-950/40 border border-indigo-500/20 flex items-center justify-center text-indigo-400 shadow-md">
                <Clock className="w-5 h-5 animate-pulse" />
              </div>
              <div>
                <h4 className="text-sm font-bold text-white font-outfit">Free Trial Remaining</h4>
                <p className="text-xs text-zinc-400 mt-0.5">
                  You are currently on a 14-day free trial. There are <span className="font-semibold text-indigo-400">{getRemainingTrialDays()} days</span> left.
                </p>
              </div>
            </div>
            {activeTab !== 'billing' && (
              <button
                onClick={() => setActiveTab('billing')}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-semibold shadow-md shadow-indigo-600/10 transition-all flex items-center gap-1.5 shrink-0 group relative z-10 cursor-pointer"
              >
        ...
        {loading ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {/* SaaS Account Suspended Overlay */}
              {(hotelConfig?.subscriptionStatus === 'past_due' || hotelConfig?.subscriptionStatus === 'cancelled') && ['dashboard', 'rooms', 'bookings'].includes(activeTab) ? (
                <div className="py-12 flex items-center justify-center">
                  <div className="max-w-md w-full bg-zinc-950/80 border border-zinc-900 rounded-2xl p-8 text-center shadow-2xl backdrop-blur-md relative overflow-hidden">
                    <div className="absolute inset-0 bg-red-500/5 rounded-2xl filter blur-3xl pointer-events-none" />
                    <div className="w-16 h-16 mx-auto rounded-full bg-red-950/40 border border-red-500/30 flex items-center justify-center text-red-500 shadow-lg shadow-red-500/10 mb-6 animate-pulse">
                    ...
                    <p className="text-sm text-zinc-400 leading-relaxed mb-6">
                      Your HotelBot SaaS trial has expired, or your recent subscription payment failed. To resume taking hotel reservations on Telegram and managing your rooms catalog, please choose a subscription plan or update your billing details.
                    </p>
                    <button
                      onClick={() => setActiveTab('billing')}
                      className="w-full py-3 px-4 bg-gradient-to-r from-red-600 to-indigo-600 hover:from-red-500 hover:to-indigo-500 text-white font-semibold rounded-lg text-sm transition-all shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-2 group cursor-pointer"
                    >
              ...
              {activeTab === 'billing' && (
                <div className="space-y-8 max-w-5xl">
                  ...
                  {hotelConfig?.subscriptionStatus === 'active' ? (
                    ...
                    <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-8 relative overflow-hidden shadow-2xl">
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                        <div className="space-y-4">
                          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/30 border border-emerald-500/30 text-xs font-semibold text-emerald-400">
                            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
                            <span>Active Subscription</span>
                          </div>
                          
                          <h2 className="text-3xl font-bold font-outfit text-white">
                            Partner Premium Plan
                          </h2>
                          
                          <p className="text-sm text-zinc-400 max-w-xl leading-relaxed">
                            Thank you for being a valued HotelBot B2B SaaS partner! Your account is active, and your Telegram chatbot is live taking direct reservations with zero commissions.
                          </p>

                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                            <div className="flex items-center gap-2">
                              <Check className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs text-zinc-300">Unlimited bookings & guests</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Check className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs text-zinc-300">Custom brand styling active</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Check className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs text-zinc-300">Priority server hosting</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <Check className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs text-zinc-300">Real-time analytical metrics</span>
                            </div>
                          </div>
                        </div>

                        <div className="flex flex-col gap-3 min-w-[240px] bg-zinc-900/40 border border-zinc-800 p-6 rounded-xl text-center">
                          <div className="text-xs text-zinc-500 font-semibold uppercase tracking-wider">Subscription Management</div>
                          <div className="text-2xl font-extrabold text-white mt-1">Stripe Billing</div>
                          <p className="text-[10px] text-zinc-500 leading-relaxed mb-4">
                            Update credit cards, download tax invoices, or modify plans securely.
                          </p>
                          <button
                            onClick={handleManageBilling}
                            disabled={billingLoading !== null}
                            className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-800/50 disabled:cursor-not-allowed text-white font-semibold rounded-lg text-xs transition-all shadow-md shadow-indigo-600/10 cursor-pointer flex items-center justify-center gap-2"
                          >
                    ...
                    <div className="space-y-8">
                      <div className="text-center max-w-xl mx-auto space-y-2">
                        <h2 className="text-2xl font-bold font-outfit text-white">Choose Your Subscription Plan</h2>
                        <p className="text-sm text-zinc-400">
                          Activate your partner license to unlock seamless chatbot booking automations and real-time dashboard operations.
                        </p>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* Starter Card */}
                        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 flex flex-col justify-between relative transition-all duration-300 hover:border-zinc-800 shadow-xl hover:shadow-indigo-500/5">
                          <div className="space-y-4">
                            <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Starter</div>
                            <div className="flex items-baseline gap-1 text-white">
                              <span className="text-4xl font-extrabold">$49</span>
                              <span className="text-zinc-500 text-xs font-semibold">/ month</span>
                            </div>
                            <p className="text-xs text-zinc-400 leading-relaxed">
                              Ideal for boutique hotels, guest houses, and small B&Bs.
                            </p>
                            ...
                            <div className="space-y-2.5">
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <span className="text-zinc-300">Up to 5 Room Categories</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Standard Telegram Bot</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Email Reservations Logs</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">14-day free trial offset</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pt-6">
                            <button
                              ...
                              disabled={billingLoading !== null}
                              className="w-full py-2.5 px-4 bg-zinc-900 hover:bg-zinc-800 disabled:bg-zinc-950 disabled:cursor-not-allowed text-zinc-300 hover:text-white font-semibold rounded-lg text-xs border border-zinc-800 hover:border-zinc-700 transition-all flex items-center justify-center gap-2 cursor-pointer"
                            >
                              {billingLoading === 'starter' ? (
                                <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                              ) : (
                                <span>Subscribe Starter</span>
                              )}
                            </button>
                          </div>
                        </div>
                        ...
                        {/* Pro Card */}
                        <div className="bg-zinc-950 border border-indigo-500/40 rounded-2xl p-6 flex flex-col justify-between relative transition-all duration-300 hover:border-indigo-500 shadow-xl shadow-indigo-500/5">
                          <div className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-[9px] font-bold uppercase tracking-wider py-1 px-3 rounded-full absolute -top-3 left-1/2 -translate-x-1/2 shadow-lg shadow-indigo-500/20">
                            Recommended
                          </div>

                          <div className="space-y-4">
                            <div className="text-xs font-bold text-indigo-400 uppercase tracking-widest">Professional</div>
                            <div className="flex items-baseline gap-1 text-white">
                              <span className="text-4xl font-extrabold">$149</span>
                              <span className="text-zinc-500 text-xs font-semibold">/ month</span>
                            </div>
                            <p className="text-xs text-zinc-400 leading-relaxed">
                              Perfect for growing mid-sized hotels, resorts, and vacation properties.
                            </p>
                            ...
                            <div className="space-y-2.5">
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <span className="text-zinc-300 font-semibold">Unlimited Room Categories</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Custom Brand Color Integration</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300 font-semibold">Real-Time Performance Dashboard</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-emerald-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300 font-semibold">Priority API Support</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pt-6">
                            <button
                              ...
                              disabled={billingLoading !== null}
                              className="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-800/50 disabled:cursor-not-allowed text-white font-semibold rounded-lg text-xs transition-all shadow-md shadow-indigo-600/10 flex items-center justify-center gap-2 cursor-pointer"
                            >
                              {billingLoading === 'pro' ? (
                                <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                              ) : (
                        ...
                        <div className="bg-zinc-950 border border-zinc-900 rounded-2xl p-6 flex flex-col justify-between relative transition-all duration-300 hover:border-zinc-800 shadow-xl hover:shadow-indigo-500/5">
                          <div className="space-y-4">
                            <div className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Enterprise</div>
                            <div className="flex items-baseline gap-1 text-white">
                              <span className="text-4xl font-extrabold">$499</span>
                              <span className="text-zinc-500 text-xs font-semibold">/ month</span>
                            </div>
                            <p className="text-xs text-zinc-400 leading-relaxed">
                              Custom setup for major luxury hotel groups and chains.
                            </p>
                            ...
                            <div className="space-y-2.5">
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Multi-Property Locations</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Custom Domains & Bot Config</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">Dedicated 24/7 Account Manager</span>
                              </div>
                              <div className="flex items-start gap-2 text-xs">
                                <Check className="w-4 h-4 text-indigo-400 mt-0.5 shrink-0" />
                                <span className="text-zinc-300">SLA Guarantee & Webhook API</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="pt-6">
                            ...
                            >
                              {billingLoading === 'enterprise' ? (
                                ...
                                <span>Subscribe Enterprise</span>
                              )}
                            </button>
                          </div>
                        </div>
